...
void menuLoop(void* menuToRun){
    ...
    while(1){
        ...
        if( key & KEY_EVENT_UP )   cursor = 0 ? cursor = currentMenu->size() -1 : cursor--;    // 光标上移
        if( key & KEY_EVENT_DOWN ) cursor = currentMenu->size() -1 ? cursor = 0 : cursor++;    // 光标下移
        if( key & KEY_EVENT_LEFT ) break;                                               // 左键返回
        if( key & KEY_EVENT_RIGHT ){currentMenu->runItem();currentMenu = nullptr;}             // 执行选项并清除菜单指针
        if( key & KEY_EVENT_A )    {currentMenu->runItem();currentMenu = nullptr;}             // 执行选项并清除菜单指针
