...
public:
    menuItem_t(String name);                // 构造函数
    ~menuItem_t();                          // 析构函数

    String getName() const;                 // 读名字
...
/**
 * @brief 将函数调用绑定到菜单项的宏
 * @param MENU_ITEM  menuItem_t 对象的指针/引用，填你要绑到的菜单项
 * @param FUNC 函数调用表达式或代码，填你要绑的函数
 * 
 ...
 * bindRun(&item, add(A, B));                          // 绑定一个函数
 * item.run();      //=7                               // 绑定完后调用run=调用绑定函数（run在选中菜单时自动调用）
 * @endcode
 */
#define bindRun(MENU_ITEM, FUNC) \
    do { \
        (MENU_ITEM)->bind = [ ]() -> int32_t { \
            return (int32_t)(FUNC); \
        }; \
